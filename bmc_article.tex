...
\begin{equation} \label{eq:struc}
    f_j\left[k\right] \approx \sum_{k'\in K'} \mu_j\left[k'\right] \phi\left[k',
      k\right] + \overline{f_j\left[k\right]}
\end{equation}
...
calibrated principle components (PCs) and the
$\overline{f_j\left[k\right]}$ are the mean values for each $j$ and
independent of $k$. The $k' \in K'$ indices refer to the
$\mu_j\left[k'\right]$ in decreasing order of significance and are
independent of $l$, $l'$ and $r$. The main advantage of this approach
is that the $f_j\left[k\right]$ can be reconstructed to sufficient
fidelity with only a small subset of the $K'$ indices [ADDREFS].
...
\begin{multline}
    \label{eq:series}
    p_i[s] = \sum_{h; r} \alpha[h; r] m_i[h; s - r] + \sum_{h, h'; r, r'}
    \alpha[h, h'; r, r'] m_i[h; s - r] m_i[h'; s - r'] + ...
\end{multline}
In Eq. \ref{eq:series}, $p_i[s]$ is the spatially resolved (localized)
